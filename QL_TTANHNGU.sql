...
CREATE TABLE HoaDon(
	...
	MaLH nchar(10) CONSTRAINT FK_HoaDon_MaLH FOREIGN KEY REFERENCES LopHoc(MaLH)
	on delete set null 
	on update cascade
);
GO
...
@DiaChiHV nvarchar(100)
AS
update HocVien set HoTenHV = @HoTenHV, NgaySinh = @NgaySinh, GioiTinh = @GioiTinh, 
		SoDT = @SoDT, DiaChiHV = @DiaChiHV where MaHV = @MaHV;

--Hiẻn thị theo mã học viên 
...
--Chi tiết đăng ký lớp học
--Thêm chi tiết đăng ký lớp học
...
--Hiển thị theo mã học viên và mã lớp học
...
--Xem thông tin Chi tiết đăng ký lớp học 
